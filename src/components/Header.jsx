...
function Header({ logo, liItems }) {
  ...
  return (
    <>
        <header className="flex items-center fixed z-50 top-0 h-28 w-full shadow-2xl bg-black gap-8 pr-4">
            <Link to={'/'} className="flex h-full items-center -skew-x-12 pr-10 md:pr-20 bg-white -ml-4">
            <div className="max-w-72 sm:w-80 md:ml-24 ml-16 skew-x-12">
                <img loading="lazy" id="logo" alt="Logo" src={logo} />
            </div>
            </Link>
            <div className="flex flex-grow">
                <div className="flex gap-4 sm:gap-8 justify-end flex-grow md:pr-8 font-light uppercase text-2xl text-slate-50 items-center">
                    <div className="hidden lg:flex gap-8  items-center">
                        {liItems.map((item, index) => (
                            <nav key={index} className="w-min cursor-pointer xl:w-full text-nowrap after:block after:border-b after:border-red-500 after:scale-0 after:transition-all hover:after:scale-100"><HashLink to={item.href}> {item.text} </HashLink></nav>
                        ))}
                    </div>
                    <button className='hidden xl:flex text-black bg-gradient-to-r from-secondary to-primary py-2 px-4 rounded-xl my-8 font-normal transition-all duration-300 hover:scale-105'><a href="tel:+420735063733">POPTEJTE NÁS</a></button>
                ...
                <div className="flex lg:hidden w-8 h-8 mr-4">
                    <HamburgerDrawer > 
                        ...
                        <button className='w-1/3 min-w-36 bg-gradient-to-r from-secondary to-primary py-2 px-4 rounded-xl my-8 font-normal transition-all duration-300 hover:scale-105'><a href="tel:+420735063733">POPTEJTE NÁS</a></button>
                        </div>
                    </HamburgerDrawer>
                </div>
            </div>
