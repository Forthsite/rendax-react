            ...
            <div>
                <div className="flex gap-4 sm:gap-8 font-light uppercase text-2xl text-[#0f121b] cursor-pointer items-center">
                    <div className="hidden lg:flex gap-8  items-center">
                        {liItems.map((item, index) => (
                            <nav key={index} className="w-min xl:w-full text-nowrap after:block after:border-b after:border-primary after:scale-0 after:transition-all hover:after:scale-100"><HashLink to={item.href}> {item.text} </HashLink></nav>
                        ))}
                    </div>
                ...
                <div className="flex lg:hidden w-8 h-8 mr-4">
                    <HamburgerDrawer > 
                        ...
                        {liItems.map((item, index) => (
                                <nav key={index} className="w-2/3 font-light uppercase text-2xl after:block after:border-b after:bg-primary after:scale-0 after:transition-all hover:after:scale-100"><a href={item.href}>{item.text}</a></nav>
                            ))}
                        </div>
                    </HamburgerDrawer>
                </div>
